...
namespace SpirvNet.Spirv.Ops.Texture
{
    ...
    public sealed class OpTextureSampleGradOffset : Instruction
    {
        public override bool IsTexture => true;
        public override OpCode OpCode => OpCode.TextureSampleGradOffset;
        ...
        {
            System.Diagnostics.Debug.Assert((codes[start] & 0x0000FFFF) == (uint)OpCode.TextureSampleGradOffset);
            var i = 1;
            ResultType = new ID(codes[start + i++]);
            Result = new ID(codes[start + i++]);
            Sampler = new ID(codes[start + i++]);
            Coordinate = new ID(codes[start + i++]);
            dx = new ID(codes[start + i++]);
            dy = new ID(codes[start + i++]);
